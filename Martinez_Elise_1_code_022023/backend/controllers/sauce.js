...
const fs = require('fs'); // file system, package qui permet la modification et/ou la supression de fichiers


// création, modification, suppression et récupération des sauces
exports.createSauce = (req, res, next) => {
    const sauceObject = JSON.parse(req.body.sauce);
    delete sauceObject._id;    
    const sauce = new Sauce({ // un nouvel objet  est créé avec le model Sauce
        ...sauceObject,
    ...
    sauce.save() // la sauce est sauvegardée en bdd
...
exports.modifySauce = (req, res, next) => {
    const sauceObject = req.file ? // vérifie si la modif concerne le body ou un nouveau fichier image
    {
...
exports.getAllSauces = (req, res, next) => { // on va récupérer toutes les sauces disponibles
    Sauce.find()
...
exports.likeSauce = (req, res, next) => {    
    ...
    } else {    // annulation des boutons j'aime/je n'aime pas
        Sauce.findOne( {_id: req.params.id})
        .then( sauce => {
